...
and it in the same ways for $\texttt{>}$, $\texttt{<=}$ and $\texttt{>=}$. An
...
In this section, we present our implementation for the L2 \gls{ir} in Elixir.

\subsection{Overview}
\label{ir:dslexample}

We have implemented a \gls{dsl} in Elixir to code and verify programs written in
the L2 \gls{ir}. The \verb|with_local_env/2| macro provides a default fresh
connection with Z3 and allows other configurations, such as a function to listen
every verification error report when verifying the given code.

Here are some examples that succeed to verify according to the Elixir behavior 
explained in \ref{prelim:elixir}:
...
defines the corresponding macros from the example at \ref{ir:dslexample}. They
translate the L1 \gls{dsl} into L0 and evaluate it as in
\ref{integ:l0implementation}. A verification function for L1 statements can be
implemented in terms of it as follows but, in contrast to the one presented in
